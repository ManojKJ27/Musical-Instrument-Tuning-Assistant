...
# Display the corrective action required
    ...
    if(closest_distance == npabs(Sa_error)):
        ...
        String="Target is Sa : "+str(target)+" Hz"
        
        if(abs(Sa_error)<2):
            print("Sa : tuned at ",target,"Hz")
            strint_in="Sa : tuned at "+str(target)+"Hz"
        else:
            if(Sa_error>0):
    ...
    if(closest_distance == npabs(Pa_error)):
        ...
        String="Pa : tuned at "+str(target*3/2)+"Hz"
        if(abs(Pa_error)<2):
    ...
    if(closest_distance == npabs(Sah_error)):
        ...
        strint_in="Sa : tuned at "+str(target*2)+"Hz"
        if(abs(Sah_error)<2):
            ...
            if(Sah_error>0):
                ...
                strint_in="Sa : Increase by "+str(Sah_error)+"Hz"
            else:
                print("Sa : Decrease by ",Sah_error," Hz")
                strint_in="Sa : Decrease by "+str(Sah_error)+"Hz"
                
    return String,strint_in